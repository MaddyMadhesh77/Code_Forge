  ...
  return (
    <AnimatedPage>
      <div className="space-y-6 text-[color:var(--text-primary)]">
        <div className="rounded-3xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-6">
        ...
        <div className="flex flex-wrap gap-2 rounded-3xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] p-2">
          {tabs.map((entry) => (
            <button
              key={entry.id}
              type="button"
              onClick={() => navigate(`/settings/${entry.id}`)}
              className={`inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold transition ${tab === entry.id ? 'bg-[color:var(--color-primary)] text-white' : 'text-[color:var(--text-muted)] hover:bg-[color:var(--bg-surface-active)] hover:text-[color:var(--text-primary)]'}`}
            >
              {entry.icon}
              {entry.label}
            </button>
        ...
        {loading && <Card className="text-[color:var(--text-muted)]">Loading settings…</Card>}

        {!loading && (!profile || !orgSettings) && (
          <Card className="text-[color:var(--text-muted)]">Unable to load settings right now. Try refreshing the page.</Card>
        )}
            ...
            <Card className="space-y-5">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="space-y-2 text-sm">
                      ...
                      <input className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={profile.displayName} onChange={(event) => setProfile({ ...profile, displayName: event.target.value })} />
                    </label>
                    <label className="space-y-2 text-sm">
                      <span className="text-[color:var(--text-muted)]">Title</span>
                      <input className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={profile.title ?? ''} onChange={(event) => setProfile({ ...profile, title: event.target.value })} />
                    </label>
                    <label className="space-y-2 text-sm">
                      <span className="text-[color:var(--text-muted)]">Phone</span>
                      <input className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={profile.phone ?? ''} onChange={(event) => setProfile({ ...profile, phone: event.target.value })} />
                    </label>
                    <label className="space-y-2 text-sm">
                      <span className="text-[color:var(--text-muted)]">Timezone</span>
                      <input className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={profile.timezone ?? ''} onChange={(event) => setProfile({ ...profile, timezone: event.target.value })} />
                    </label>
                  </div>
              ...
              {tab === 'security' && (
                <div className="space-y-4">
                  ...
                  <label className="flex items-center justify-between rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 text-sm">
                    <div>
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <label className="space-y-2 text-sm">
                      ...
                      <input type="password" className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={passwordFields.currentPassword} onChange={(event) => setPasswordFields({ ...passwordFields, currentPassword: event.target.value })} />
                    </label>
                    <label className="space-y-2 text-sm">
                      <span className="text-[color:var(--text-muted)]">New password</span>
                      <input type="password" className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={passwordFields.newPassword} onChange={(event) => setPasswordFields({ ...passwordFields, newPassword: event.target.value })} />
                    </label>
                    <label className="space-y-2 text-sm">
                      <span className="text-[color:var(--text-muted)]">Confirm</span>
                      <input type="password" className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={passwordFields.confirmPassword} onChange={(event) => setPasswordFields({ ...passwordFields, confirmPassword: event.target.value })} />
                    </label>
                  </div>
                  <button className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 text-sm font-semibold text-[color:var(--text-primary)] transition hover:bg-[color:var(--bg-surface-active)]" type="button" onClick={() => setToast('Password flow staged for the backend contract')}>
              ...
              {tab === 'keys' && (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <input className="min-w-0 flex-1 rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" placeholder="Key name" value={keyName} onChange={(event) => setKeyName(event.target.value)} />
                  ...
                  <div className="space-y-3">
                    {apiKeys.map((key) => (
                      <div key={key.id} className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] p-4">
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          ...
                          <div className="flex gap-2">
                            <button className="rounded-xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)]" type="button" onClick={() => void copyPrefix(key.prefix)}>
                              <Copy size={14} className="inline-block" /> Copy
                            </button>
                            <button className="rounded-xl border border-[rgba(239,68,68,0.2)] bg-[rgba(239,68,68,0.1)] px-3 py-2 text-xs font-semibold text-[color:var(--status-error)]" type="button" onClick={() => void revokeKey(key)}>
                              Revoke
                            </button>
                          </div>
                        </div>
              ...
              {tab === 'billing' && (
                <div className="space-y-4">
                  ...
                  <Card className="!bg-[color:var(--bg-surface-hover)]">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="rounded-full border border-[rgba(52,211,153,0.2)] bg-[rgba(52,211,153,0.12)] px-3 py-1 text-xs font-semibold text-[color:var(--status-success)]">Active</div>
                    </div>
                    <div className="mt-4 grid gap-3 md:grid-cols-3 text-sm text-[color:var(--text-muted)]">
                      <div className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-3">25 seats</div>
                      <div className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-3">90 day retention</div>
                      <div className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-3">CSV / ZIP exports</div>
                    </div>
                  </Card>
                </div>
              )}
        ...
        <Modal open={showKeyModal} onClose={() => setShowKeyModal(false)} title="New API key">
          <div className="space-y-4 text-[color:var(--text-primary)]">
            ...
            <div className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] p-4 font-mono text-sm break-all">{newKeySecret}</div>
            <div className="flex justify-end gap-3">
              <button className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-2 text-sm text-[color:var(--text-primary)]" type="button" onClick={() => setShowKeyModal(false)}>
                Close
              </button>
