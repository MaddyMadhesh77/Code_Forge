...
function renderChart(series: UsageMetrics['series']) {
  ...
  return (
    <svg viewBox="0 0 100 100" className="h-72 w-full overflow-visible rounded-3xl border border-white/10 bg-[#0a0f1a] p-3">
      <defs>
        <linearGradient id="usageGradient" x1="0" x2="0" y1="0" y2="1">
          <stop offset="0%" stopColor="rgba(94,123,255,0.7)" />
          <stop offset="100%" stopColor="rgba(94,123,255,0.05)" />
        </linearGradient>
      </defs>
      <path d={path} fill="url(#usageGradient)" opacity="0.8" />
      <polyline fill="none" stroke="rgba(122,161,255,0.95)" strokeWidth="2.5" points={points.map((point) => `${point.x},${point.y}`).join(' ')} />
      {points.map((point, index) => (
        <circle key={series[index].date} cx={point.x} cy={point.y} r="1.5" fill="white" opacity="0.9" />
      ))}
    </svg>
  );
}
    ...
    <AnimatedPage>
      <div className="space-y-6 text-white">
        <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-white/10 bg-[#0a0f1a] p-2">
            {RANGE_OPTIONS.map((option) => (
              <button key={option} type="button" onClick={() => setRange(option)} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${range === option ? 'bg-[color:var(--color-primary)] text-white' : 'text-[color:var(--text-muted)] hover:bg-white/5 hover:text-white'}`}>
                {option}
              </button>
            ))}
            <button className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/10" type="button" onClick={() => void load()}>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => (
            <Card key={stat.label} className="border-white/10 bg-white/5 p-5 text-white">
        ...
        {loading && <Card className="border-white/10 bg-white/5 p-6 text-[color:var(--text-muted)]">Loading metrics…</Card>}
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,0.9fr)]">
            <Card className="space-y-4 border-white/10 bg-white/5 p-5 text-white">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="rounded-xl border border-white/10 bg-[#0a0f1a] px-3 py-2 text-sm text-[color:var(--text-muted)]">
                  {usage.range}
                </div>
              </div>
            ...
            <Card className="space-y-4 border-white/10 bg-white/5 p-5 text-white">
              ...
              <label className="space-y-2 text-sm">
                ...
                <select className="w-full rounded-2xl border border-white/10 bg-[#0a0f1a] px-4 py-3 outline-none" value={service} onChange={(event) => setService(event.target.value as typeof service)}>
              ...
              <label className="space-y-2 text-sm">
                ...
                <select className="w-full rounded-2xl border border-white/10 bg-[#0a0f1a] px-4 py-3 outline-none" value={exportFormat} onChange={(event) => setExportFormat(event.target.value as typeof exportFormat)}>
                  {EXPORT_OPTIONS.map((option) => (
                    <option key={option} value={option} className="text-black">{option.toUpperCase()}</option>
                  ))}
                </select>
              </label>
              ...
              <div className="rounded-2xl border border-white/10 bg-[#0a0f1a] p-4 text-sm text-[color:var(--text-muted)]">
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <Card className="space-y-4 border-white/10 bg-white/5 p-5 text-white">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-[color:var(--text-muted)]">{topProblems.length} items</span>
            </div>
            <div className="space-y-3">
              {topProblems.map((problem, index) => (
                <div key={problem.id} className="rounded-2xl border border-white/10 bg-[#0a0f1a] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
          ...
          <Card className="space-y-4 border-white/10 bg-white/5 p-5 text-white">
            ...
            <div className="space-y-3">
              {errors.map((error) => (
                <div key={error.id} className="rounded-2xl border border-white/10 bg-[#0a0f1a] p-4">
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <div>
                      <div className="font-semibold text-white">{error.message}</div>
