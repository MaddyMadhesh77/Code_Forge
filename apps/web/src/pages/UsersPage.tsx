...
export default function UsersPage() {
  ...
  const statusStyles: Record<UserStatus, string> = {
    ACTIVE: 'bg-[rgba(52,211,153,0.12)] text-[color:var(--status-success)] border-[rgba(52,211,153,0.2)]',
    INVITED: 'bg-[rgba(255,178,36,0.12)] text-[color:var(--status-warning)] border-[rgba(255,178,36,0.2)]',
    SUSPENDED: 'bg-[rgba(239,68,68,0.12)] text-[color:var(--status-error)] border-[rgba(239,68,68,0.2)]',
  };
      ...
      <div className="space-y-6 text-[color:var(--text-primary)]">
        <div className="flex flex-col gap-4 rounded-3xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 text-sm text-[color:var(--text-primary)]">
        ...
        <Card className="space-y-4">
          <div className="grid gap-3 md:grid-cols-[220px_minmax(0,1fr)]">
            <label className="flex items-center gap-2 rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 text-sm text-[color:var(--text-muted)]">
              <Users size={16} />
              <select className="w-full bg-transparent text-[color:var(--text-primary)] outline-none" value={role} onChange={(event) => setRole(event.target.value as Role | '')}>
            ...
            <label className="flex items-center gap-2 rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 text-sm text-[color:var(--text-muted)]">
              <Search size={16} />
              <input className="w-full bg-transparent text-[color:var(--text-primary)] outline-none placeholder:text-[color:var(--text-muted)]" placeholder="Search by name or email" value={query} onChange={(event) => setQuery(event.target.value)} />
            </label>
          </div>

          <div className="overflow-hidden rounded-3xl border border-[color:var(--border-primary)]">
            <div className="grid grid-cols-[1.2fr_1.4fr_0.9fr_0.9fr_0.9fr_0.8fr] gap-3 bg-[color:var(--bg-surface-hover)] px-4 py-3 text-xs uppercase tracking-[0.22em] text-[color:var(--text-muted)]">
              <div>Avatar</div>
              <div>Name</div>
              <div>Email</div>
              <div>Role</div>
                ...
                {users.map((user) => (
                  <div key={user.id} className="grid grid-cols-[1.2fr_1.4fr_0.9fr_0.9fr_0.9fr_0.8fr] items-center gap-3 px-4 py-4 text-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-[color:var(--text-primary)]">{user.displayName}</div>
                    ...
                    <select
                      className="rounded-xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-3 py-2 text-[color:var(--text-primary)] outline-none"
                      value={user.role}
                    ...
                    <div className="flex justify-end gap-2">
                      <button className="rounded-xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] px-3 py-2 text-xs font-semibold text-[color:var(--text-primary)] transition hover:bg-[color:var(--bg-surface-hover)]" type="button" onClick={() => void openUser(user)}>
                        Details
                      </button>
                      <button className="rounded-xl border border-[rgba(239,68,68,0.2)] bg-[rgba(239,68,68,0.1)] px-3 py-2 text-xs font-semibold text-[color:var(--status-error)] transition hover:bg-[rgba(239,68,68,0.18)]" type="button" onClick={() => setPendingDelete(user)}>
        ...
        {selectedUser && (
          <motion.aside initial={{ x: 36, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="fixed right-0 top-0 z-40 h-full w-full max-w-[420px] border-l border-[color:var(--border-primary)] bg-[color:var(--bg-surface)] p-6 text-[color:var(--text-primary)] shadow-[-20px_0_80px_rgba(0,0,0,0.18)] backdrop-blur">
            <div className="flex items-start justify-between gap-3">
              ...
              <button className="rounded-xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-3 py-2 text-xs text-[color:var(--text-primary)]" type="button" onClick={() => setSelectedUser(null)}>
                Close
              </button>
            </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <Card>
                ...
                <div className="mt-4 space-y-3">
                  {activity.map((entry) => (
                    <div key={entry.id} className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] p-3">
                      <div className="flex items-center justify-between text-xs text-[color:var(--text-muted)]">
                        <span>{entry.action}</span>
                        <span>{formatDate(entry.timestamp)}</span>
                      </div>
                      <p className="mt-2 text-sm text-[color:var(--text-secondary)]">{entry.details}</p>
                    </div>
                  ))}
                </div>
              </Card>
        ...
        <Modal open={inviteOpen} onClose={() => setInviteOpen(false)} title="Invite user">
          <div className="space-y-4 text-[color:var(--text-primary)]">
            <div>
              ...
              <input className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="name@company.com" />
              {inviteError && <p className="mt-2 text-sm text-[color:var(--status-error)]">{inviteError}</p>}
            </div>
            <div>
              <label className="mb-2 block text-sm text-[color:var(--text-muted)]">Role</label>
              <select className="w-full rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-3 outline-none" value={inviteRole} onChange={(event) => setInviteRole(event.target.value as Role)}>
                <option value="ADMIN">Admin</option>
                <option value="INTERVIEWER">Interviewer</option>
                <option value="CANDIDATE">Candidate</option>
              </select>
            ...
            <div className="flex justify-end gap-3">
              <button className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-2 text-sm text-[color:var(--text-primary)]" type="button" onClick={() => setInviteOpen(false)}>
                Cancel
              </button>
        ...
        <Modal open={Boolean(pendingDelete)} onClose={() => setPendingDelete(null)} title="Remove user">
          <div className="space-y-4 text-[color:var(--text-primary)]">
            ...
            <div className="flex justify-end gap-3">
              <button className="rounded-2xl border border-[color:var(--border-primary)] bg-[color:var(--bg-surface-hover)] px-4 py-2 text-sm text-[color:var(--text-primary)]" type="button" onClick={() => setPendingDelete(null)}>
                Cancel
              </button>
              <button className="rounded-2xl bg-[color:var(--status-error)] px-4 py-2 text-sm font-semibold text-white" type="button" onClick={() => void handleDelete()}>
                Confirm remove
              </button>
            </div>
          </div>
